...
public class StandardRename {
    ...
    public static void startRenamingProcess(int count, int sizeOfRenamingList, Student[] student, ToRename[] rename,
        ...
        while (count < listOfFilesToBeRenamed.length - 1) {
            ...
            try {
                if (!valid && !listOfFilesToBeRenamed[count].contains(".csv")
                        && listOfFilesToBeRenamed[count].contains(".pdf") && SimpleRename.valid.equals(false)
                        && ComplexRename.valid.equals(false)) {
    ...
    public String[] getListOfFilesToBeRenamed(){
        return StandardRename.listOfFilesToBeRenamed;
    }
    
    public String[] getFirstSplitList(){
        return StandardRename.firstSplitList;
    }
    public String[] getInvalidSubmissions(){
        return StandardRename.invalidSubmissions;
    }
    public int getNumeral(){
        return StandardRename.numeral;
    }
