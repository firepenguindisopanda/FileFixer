...
public class Rename {
    ...
    public void startRename(Student[] student, ToRename[] rename, FileCollection fcollection) {
        ...
        SimpleRename.startRenamingProcess(count, filesToRename.getToBeRenamedList().size(), student, rename,
                fcollection);
        StandardRename.startRenamingProcess(count, filesToRename.getToBeRenamedList().size(), student, rename,
                fcollection);
        ComplexRename.startRenamingProcess(count, filesToRename.getToBeRenamedList().size(), student, rename,
                fcollection);
        count++;
    ...
    public static int checkValidity(Boolean valid, int numeral, String[] listOfFilesToBeRenamed, int count) {
        ...
        if (valid.equals(false) && !listOfFilesToBeRenamed[count].contains(".csv")
                && listOfFilesToBeRenamed[count].contains(".pdf")) {
    ...
    public static void generateTXT(int count, int sizeOfRenamingList, Boolean valid, int numeral,
            String[] invalidSubmissions, Student[] student, String[] listOfFilesToBeRenamed) {
            ...
            while (c < getLength(student)) {
                ...
                while (counter < numeral) {
                    if (invalid[counter].contains(s1.getname()) || invalid[counter].contains(s1.getID())
                            || invalid[counter].contains(s1.getname().toLowerCase())
                            || invalid[counter].contains(s1.getname().toUpperCase())
                            || invalid[counter].contains(s1.getname().replace(" ", ""))) {
                        invalid[counter] = "";
                    }
                    counter++;
                }
